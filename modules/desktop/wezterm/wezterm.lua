...
local function stylix_wrapped_config()
  ...
  wezterm.on('window-focus-changed', function(window, pane)
    ...
    local file_comp = io.open("/tmp/WEZTERM_CMD_COMP", 'r+')
    local read_comp = file_comp:read("*all")
    wezterm.log_info(read_comp)

    file_comp:close()
    file:close()
    
    wezterm.log_info(read,read_comp,read == read_comp)
    if read == read_comp then
      return
    end

    local file_write = io.open("/tmp/WEZTERM_CMD_COMP", 'w')
    file_write:write(read)
    file_write:close()

    local workspace,cwd,cmd,random = table.unpack(split(read,'@'))
    wezterm.log_info(workspace, cwd, cmd, random)
